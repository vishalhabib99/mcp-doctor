...
`mark3labs/mcp-go`'s older fluent-builder style (`mcp.NewTool(name,
mcp.WithDescription(...), mcp.WithString(...))`) is real and still used in
some production servers (see `github/github-mcp-server`'s own migration
guide off of it), but isn't supported yet — see Known limitations in the
README. Not guessing at an unverified API shape here follows the same
principle used throughout this analyzer: verify against real source before
shipping a check.
