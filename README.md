...
- [x] TypeScript/JS server support (the official SDK's dominant language) — `registerTool`/`tool` styles, cross-file const/member-expression resolution, plus the community `fastmcp` package's `addTool` single-object style
- [x] Publish to PyPI
